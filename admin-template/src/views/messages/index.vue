...
<template>
  <div class="app-container">
      ...
      <el-form :model="where" @submit.native.prevent>
        <el-col :span="5">
          <el-form-item label="父级栏目">
            <el-select v-model="where.columns_id" placeholder="请选择父级栏目" @change="setWhere">
        ...
        <el-col :span="5">
          <el-form-item label="首页推荐">
            <el-select v-model="where.index" placeholder="请选择首页推荐" @change="setWhere">
        ...
        <el-col :span="5">
          <el-form-item label="热门推荐">
            <el-select v-model="where.hot" placeholder="请选择热门推荐" @change="setWhere">
              <el-option label="全部" value="" />
              <el-option label="是" value="1" />
              <el-option label="否" value="0" />
            </el-select>
        ...
        <el-col :span="5">
          <el-form-item label="发布状态">
            <el-select v-model="where.release" placeholder="请选择发布状态" @change="setWhere">
              <el-option label="全部" value="" />
              <el-option label="是" value="1" />
              <el-option label="否" value="0" />
            </el-select>
      ...
      <el-table-column align="center" label="操作" width="180">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-button type="primary" icon="el-icon-picture" circle @click="editImagesDialog(scope.row.id, scope.row.name)" />
            <el-button type="success" icon="el-icon-postcard" circle @click="editContentDialog(scope.row.id, scope.row.name)" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" :before-close="dialogBeforeClosed" width="60%" center>
      <el-form class="edit-dialog-form">
        ...
        <el-form-item label="修改时间">
          <el-date-picker v-model="editInfo.update_time" type="datetime" placeholder="选择修改时间" />
        </el-form-item>
        <el-form-item label="发布时间">
          <el-date-picker v-model="editInfo.release_time" type="datetime" placeholder="选择发布时间" />
        </el-form-item>
        <el-form-item label="页面">
          <el-select v-model="editInfo.page" placeholder="页面" filterable>
            <el-option v-for="item in viewsList" :key="item.path" :label="item.name" :value="item.path" />
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :title="editContent.title" :visible.sync="editContent.visibleMarkdown" width="60%" :before-close="editContentHandleClose" class="dialog-visible-content">
      <markdown :content-markdown="editContent.content.markdown" :name="editContent.contentImageName" :path="editContent.contentImagePath" @setContent="editContentSet" />
      <span slot="footer">
    ...
    <el-dialog :modal="false" :title="editContent.title" :visible.sync="editContent.visibleTinyMCE" width="60%" :before-close="editContentHandleClose" class="dialog-visible-content-tiny-mce">
      <tiny-mce-editor :content="editContent.content.content" :name="editContent.contentImageName" :path="editContent.contentImagePath" @setContent="editContentSet" />
      <span slot="footer">
        <el-button type="primary" @click="editContentOrImagesSubmit">确 定</el-button>
      </span>
    </el-dialog>
    ...
    getContent(id, callback) {
      ...
      content(that.editContent.content).then(res => {
        ...
        if (res.data.images !== null && res.data.images.length > 0) {
          ...
          for (const index in that.editContent.content.images) {
            const imageName = that.editContent.content.images[index].split('/')
