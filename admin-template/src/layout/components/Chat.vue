...
/* eslint-disable prefer-const */
<template>
  <div v-show="chat === true" class="chat">
    <div class="top">
      <div class="content">用户留言列表 <span class="close" @click="close"><i class="el-icon-close" /></span> </div>
    </div>
    <div ref="chat" class="content">
      <div v-show="userListStatus === true" class="user-list">
        <div v-for="item in users" :key="item.user" class="item" @click="chatCurrent(item)">
          <span :key="item.id" v-text="'用户' + item.id" />
          <el-divider :key="item.id" content-position="right">{{ item.time | timeFilter }} </el-divider>
        </div>
        <div v-show="userWhereload === true && userWhereStatus == false" class="bottom-loading"><i class="el-icon-loading" />加载中...</div>
        <div v-show="userWhereStatus == true" class="bottom-loading">加载完成</div>
      </div>
      <div v-show="chatListStatus === true" class="chat-list">
        <div class="user"><el-page-header content="留言记录" @back="toUserList" /></div>
        <div ref="chat_list" :class="userCheck ? 'list' : 'list-total'">
          <div v-for="item in chats" :key="item.id" class="item">
            <span :key="item.id" v-text="(item.speak.length === 32 ? '用户' : '管理员') + '：' +item.content" />
            <el-divider :key="item.id" content-position="right">{{ item.add_time | timeFilter }} </el-divider>
          </div>
        </div>
        <div v-show="userCheck === true" class="speak">

          <el-form class="form">
            <el-input v-model="speakContent" type="textarea" class="speak-content" :rows="rows" placeholder="聊天内容" />
...
export default {
  ...
  watch: {
    chat(value) {
      if (value === true) { this.$refs.chat.addEventListener('scroll', this.userScrollBottom) }
      this.userWhereStatus = false
      this.userListStatus = true
      this.chatListStatus = false
      this.userWhereload = false
      this.userWhere.page = 1
      this.users = []
      this.getUsers()
    },
    user(value) {
  ...
  methods: {
    ...
    getChats() {
      ...
      GetChats(that.chatWhere).then(res => {
        ...
        for (const index in res.data.list) { that.chats.unshift(res.data.list[index]) }
