...
class Messages extends BaseModel implements ModelInterface
{
    ...
    {
        $tags = DB::table('messages_tags')->where('unique', $unique)->leftJoin(Tags::GetAlias(true), 'messages_tags.tag_id', '=', Tags::GetAlias().'id')->select(Tags::GetAlias().'id as tid', Tags::GetAlias().'name as tname')->orderBy(Tags::GetAlias().'id', 'DESC')->get();
